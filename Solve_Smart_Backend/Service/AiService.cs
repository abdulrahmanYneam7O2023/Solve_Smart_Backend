...
3.  إذا كان الكود صحيحًا  بس عاوزك تتجاهل سينتكس jeson    وتقبل المطلوب الداخلك بالضبط لأني هبعتهولك في جسون بس بسينتكس اللغة المطلوب  بمعنى أصح إعمل الحل صح لو لقيت الحل المطلوب في المسألة داخل بالضبط(CORRECT)، اكتب فقط 'الكود صحيح' بدون تعليقات إضافية أو حل صحيح.
...
-  نقطة أخيرة رجع التفاصيل دي كلها الفيد باك والحل كلو باللغة الإنجليزية
   - الحل الصحيح: قدم الحل الصحيح في نفس اللغة داخل code block (```).";
        ...
        private AiEvaluationResult ParseAiResponse(string aiMessage)
        {
            ...
            if (result.IsCorrect)
            {
                result.Feedback = "The code is clean";
